...
object ScaldingRunner {
  ...
  /**
    ...
    */
  val servingStore =
    MemcacheStore.typed[String, (BatchID, Long)](MemcacheStore.defaultClient("memcached", MemcachedConnect), "scaldingLookCount")
  ...
  def runOnce() = {
    ...
    SequenceFileReader[String,Long](sequenceFile(batch)) { (k, bAndV) =>
  ...
  def lookupHDFS() {
    ...
    SequenceFileReader[String,Long](sequenceFile(batcher.currentBatch)) { (k, bAndV) =>
  ...
  def lookup(lookId: String): Option[(BatchID,Long)] =
    Await.result {
