...
object Redis {
  ...
  def keyEncoder[T](namespace: String)(implicit inj: Codec[T]): T => ChannelBuffer = { key =>
    //val inj = Injection.connect[T, Array[Byte], ChannelBuffer]
    val inj = Injection.connect[T, Array[Byte], ChannelBuffer]
    inj(key)
  }

//  def keyEncoder[K: Codec]: K => ChannelBuffer = {
//    key =>
//      val inj=Injection.connect[K,Array[Byte], ChannelBuffer]
//      inj(key)
