...
import com.twitter.summingbird.online.option.{SummerParallelism, FlatMapParallelism}
...
object StormRunner {
  /**
    ...
    */
  import Serialization._, ViewCount._, Redis._
...
//    MemcacheStore.mergeable[(Long, BatchID), Long](MemcacheStore.defaultClient("memcached", MemcachedConnect), "stormLookCount")
  ...
  lazy val viewCountStore =
    Redis.mergeable[(Long, BatchID), Long]("productcount")

  /**
    * the param to store is by name, so this is still not created created
    * yet
    */
  val storeSupplier: MergeableStoreFactory[(Long, BatchID), Long] = Storm.store(viewCountStore)
  //val storeSupplier: StoreFactory[(Long, BatchID), Long] = Storm.store(viewCountStore)
...
//                      .set(SpoutParallelism(2))
      ...
      override def graph = viewCount[Storm](spout.asInstanceOf[BaseRichSpout], storeSupplier)
    }
  }
